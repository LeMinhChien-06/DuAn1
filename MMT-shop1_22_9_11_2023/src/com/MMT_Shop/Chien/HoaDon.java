package com.MMT_Shop.Chien;

public class HoaDon {

    private int id;
    private String maHD;
    private String maNV;
    private String tenNV;
    private String maKH;
    private String tenKH;
    private int htThanhToan;
    private float tienMat;
    private String hinhThucMua;
    private float chuyuenKhoan;
    private float traLai;
    private String voucher;
    private String trangThai;
    private String ngayGiaoDich;
    private String ngayHoanThanh;
    private String diaChi;
    private String sdt;
    private String sdtNguoiShip;
    private String tenNGuoiShip;
    private float tongTien;
    private int soLuongSP;
    private String hinhThucThanhToan;
    private String created_At;
    private String updated_At;
    private String created_By;
    private String updated_By;
    private String Deleted;

    public HoaDon() {
    }

    public HoaDon(int id, String maHD, String maNV, String tenNV, String maKH, String tenKH, int htThanhToan, float tienMat, String hinhThucMua, float chuyuenKhoan, float traLai, String voucher, String trangThai, String ngayGiaoDich, String ngayHoanThanh, String diaChi, String sdt, String sdtNguoiShip, String tenNGuoiShip, float tongTien, int soLuongSP, String hinhThucThanhToan, String created_At, String updated_At, String created_By, String updated_By, String Deleted) {
        this.id = id;
        this.maHD = maHD;
        this.maNV = maNV;
        this.tenNV = tenNV;
        this.maKH = maKH;
        this.tenKH = tenKH;
        this.htThanhToan = htThanhToan;
        this.tienMat = tienMat;
        this.hinhThucMua = hinhThucMua;
        this.chuyuenKhoan = chuyuenKhoan;
        this.traLai = traLai;
        this.voucher = voucher;
        this.trangThai = trangThai;
        this.ngayGiaoDich = ngayGiaoDich;
        this.ngayHoanThanh = ngayHoanThanh;
        this.diaChi = diaChi;
        this.sdt = sdt;
        this.sdtNguoiShip = sdtNguoiShip;
        this.tenNGuoiShip = tenNGuoiShip;
        this.tongTien = tongTien;
        this.soLuongSP = soLuongSP;
        this.hinhThucThanhToan = hinhThucThanhToan;
        this.created_At = created_At;
        this.updated_At = updated_At;
        this.created_By = created_By;
        this.updated_By = updated_By;
        this.Deleted = Deleted;
    }

    public String getSdtNguoiShip() {
        return sdtNguoiShip;
    }

    public void setSdtNguoiShip(String sdtNguoiShip) {
        this.sdtNguoiShip = sdtNguoiShip;
    }

    public String getTenNGuoiShip() {
        return tenNGuoiShip;
    }

    public void setTenNGuoiShip(String tenNGuoiShip) {
        this.tenNGuoiShip = tenNGuoiShip;
    }

   

    public String getHinhThucMua() {
        return hinhThucMua;
    }

    public void setHinhThucMua(String hinhThucMua) {
        this.hinhThucMua = hinhThucMua;
    }

   
    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getMaHD() {
        return maHD;
    }

    public void setMaHD(String maHD) {
        this.maHD = maHD;
    }

    public String getMaNV() {
        return maNV;
    }

    public void setMaNV(String maNV) {
        this.maNV = maNV;
    }

    public String getTenNV() {
        return tenNV;
    }

    public void setTenNV(String tenNV) {
        this.tenNV = tenNV;
    }

    public String getMaKH() {
        return maKH;
    }

    public void setMaKH(String maKH) {
        this.maKH = maKH;
    }

    public String getTenKH() {
        return tenKH;
    }

    public void setTenKH(String tenKH) {
        this.tenKH = tenKH;
    }

    public int getHtThanhToan() {
        return htThanhToan;
    }

    public void setHtThanhToan(int htThanhToan) {
        this.htThanhToan = htThanhToan;
    }

    public float getTienMat() {
        return tienMat;
    }

    public void setTienMat(float tienMat) {
        this.tienMat = tienMat;
    }

    public float getChuyuenKhoan() {
        return chuyuenKhoan;
    }

    public void setChuyuenKhoan(float chuyuenKhoan) {
        this.chuyuenKhoan = chuyuenKhoan;
    }

    public float getTraLai() {
        return traLai;
    }

    public void setTraLai(float traLai) {
        this.traLai = traLai;
    }

    public String getVoucher() {
        return voucher;
    }

    public void setVoucher(String voucher) {
        this.voucher = voucher;
    }

    public String getTrangThai() {
        return trangThai;
    }

    public void setTrangThai(String trangThai) {
        this.trangThai = trangThai;
    }

    public String getNgayGiaoDich() {
        return ngayGiaoDich;
    }

    public void setNgayGiaoDich(String ngayGiaoDich) {
        this.ngayGiaoDich = ngayGiaoDich;
    }

    public String getNgayHoanThanh() {
        return ngayHoanThanh;
    }

    public void setNgayHoanThanh(String ngayHoanThanh) {
        this.ngayHoanThanh = ngayHoanThanh;
    }

    public String getDiaChi() {
        return diaChi;
    }

    public void setDiaChi(String diaChi) {
        this.diaChi = diaChi;
    }

    public String getSdt() {
        return sdt;
    }

    public void setSdt(String sdt) {
        this.sdt = sdt;
    }

    public float getTongTien() {
        return tongTien;
    }

    public void setTongTien(float tongTien) {
        this.tongTien = tongTien;
    }

    public int getSoLuongSP() {
        return soLuongSP;
    }

    public void setSoLuongSP(int soLuongSP) {
        this.soLuongSP = soLuongSP;
    }

    public String getHinhThucThanhToan() {
        return hinhThucThanhToan;
    }

    public void setHinhThucThanhToan(String hinhThucThanhToan) {
        this.hinhThucThanhToan = hinhThucThanhToan;
    }

    public String getCreated_At() {
        return created_At;
    }

    public void setCreated_At(String created_At) {
        this.created_At = created_At;
    }

    public String getUpdated_At() {
        return updated_At;
    }

    public void setUpdated_At(String updated_At) {
        this.updated_At = updated_At;
    }

    public String getCreated_By() {
        return created_By;
    }

    public void setCreated_By(String created_By) {
        this.created_By = created_By;
    }

    public String getUpdated_By() {
        return updated_By;
    }

    public void setUpdated_By(String updated_By) {
        this.updated_By = updated_By;
    }

    public String getDeleted() {
        return Deleted;
    }

    public void setDeleted(String Deleted) {
        this.Deleted = Deleted;
    }

  

   

}
